...
func parseStanza(c *caddy.Controller) (*Forward, error) {
	...
	// Enforce the upstream limit against the number of declared TO entries
	// (each entry counts as 1, regardless of how many IPs a resolv.conf file
	// or a hostname target resolves to at runtime).
	if len(to) > max {
		return f, fmt.Errorf("more than %d TOs configured: %d", max, len(to))
	}
...
func splitToTargets(to []string) (ipTargets []string, hostnames []hostnameEntry, err error) {
	for _, t := range to {
		...
		if addr == "" {
			return nil, nil, fmt.Errorf("invalid upstream address %q: empty address", t)
		}
		...
		// '%' is not valid in DNS hostnames (it is only meaningful in IPv6
		// zone IDs). Reject here rather than failing silently at runtime.
		if strings.Contains(host, "%") {
			return nil, nil, fmt.Errorf("invalid upstream address %q: percent sign (%%) is not allowed in DNS hostnames", t)
		}
...
func fileExists(s string) bool {
	...
	// Treat any error other than "does not exist" as "file exists" so downstream
	// file parsing can surface a more specific error.
